    ...
    <>
      <div className='h-full flex flex-col items-center justify-start md:justify-center text-white relative backdrop-blur-xl'>
        ...
        <div>
          <AnimatePresence>
            {newfilefolder && (
              <motion.div ref={newfilefolderRef} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} style={{ transformOrigin: "bottom right" }} transition={{ type: "spring", stiffness: 400, damping: 25 }} className='flex flex-col gap-2 bg-zinc-800 border p-2 rounded-xl absolute right-16 bottom-28'>
          ...
          <motion.div whileTap={{ scale: 0.9 }} className='absolute bottom-10 right-10'>
            <Button className="bg-teal-500 rounded-full aspect-square text-black p-6 hover:bg-teal-400 transition-colors" onClick={() => setnewfilefolder(prev => !prev)}>
              <FaPlus />
            </Button>
          </motion.div>
        </div>
